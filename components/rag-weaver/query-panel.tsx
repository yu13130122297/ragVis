    ...
    <div className="h-full flex flex-col bg-card/50 rounded border border-border overflow-hidden">
      ...
      <div className="px-2 py-1 border-b border-border bg-muted/30 flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-medium text-foreground">Query</span>
        </div>
        {isLoading && (
          <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
            <div className="w-1 h-1 rounded-full bg-primary animate-pulse" />
            {isRetrieving ? "Retrieving..." : isGenerating ? "Generating..." : "..."}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 p-1.5 flex flex-col gap-1">
        ...
        <div className="flex items-center justify-between">
          ...
          {session && (
            <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
              <span><span className="text-primary font-medium">{session.retrievedChunks.length}</span> chunks</span>
              <span><span className="text-[#95e679] font-medium">{session.lockedChunks.length}</span> locked</span>
            </div>
          )}
        ...
        {!currentQuery && !isLoading && (
          <div className="mt-1">
            <span className="text-[9px] text-muted-foreground block mb-1">Try:</span>
